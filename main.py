...
class GEODataManager:
    ...
    @staticmethod
    def save_to_csv(merged_data, mapping_df, merged_file='GSE44077_merged_data.csv', mapping_file='GSE44077_gene_mapping.csv'):
...
class GEODataParser:
    ...
    def add_labels_to_expression_data(self, expression_data, metadata_key='characteristics_ch1', label_prefix=None, separator=':'):
        ...
        expression_data_transposed = expression_data.set_index('ID_REF').transpose().reset_index()
        expression_data_transposed.rename(columns={'index': 'Sample'}, inplace=True)
        
        # Merge labels with the expression data
        merged_data = pd.merge(expression_data_transposed, labels_df, on='Sample', how='left')
...
updated_expression_data = parser.add_labels_to_expression_data(cleaned_data , metadata_key='characteristics_ch1', label_prefix='tissue', separator=':')


manager.save_to_csv(merged_data=cleaned_data, mapping_df=mapping_df)
